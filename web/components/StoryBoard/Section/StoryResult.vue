  ...
  <div v-if="storyResult" class="flex flex-col gap-3">
    ...
    <div class="container" v-auto-animate>
      ...
      <div
        ...
        v-if="storyResult.data.evaluation_score && isScored"
      >
        <div
          class="w-1/4 p-4 border border-gray-950/[.1] bg-gray-950/[.01] p-4 hover:bg-gray-950/[.05] dark:border-gray-50/[.1] dark:bg-gray-50/[.10] dark:hover:bg-gray-50/[.15] shadow-md rounded-lg text-center"
        >
          <h3 class="text-xl font-semibold text-gray-700">BLEU</h3>
          <p class="text-2xl font-bold text-blue-600">
            {{
              Math.max(
                Number(storyResult.data.evaluation_score.BLEU.toFixed(3)),
                0.001
              )
            }}
          </p>
          <div class="w-full bg-gray-200 rounded-full h-4">
            <div
              ...
              :style="{
                width: `${storyResult.data.evaluation_score.BLEU * 100}%`,
              }"
            ></div>
          </div>
        </div>
        ...
        >
          ...
          <p class="text-2xl font-bold text-green-600">
            {{ storyResult.data.evaluation_score.Coherence.toFixed(3) }}
          </p>
          <div class="w-full bg-gray-200 rounded-full h-4">
            <div
              class="bg-green-600 h-4 rounded-full"
              :style="{
                width: `${storyResult.data.evaluation_score.Coherence * 100}%`,
              }"
            ></div>
          </div>
        </div>
        ...
        >
          ...
          <p class="text-2xl font-bold text-purple-600">
            {{ storyResult.data.evaluation_score.Relevance.toFixed(3) }}
          </p>
          <div class="w-full bg-gray-200 rounded-full h-4">
            <div
              class="bg-purple-600 h-4 rounded-full"
              :style="{
                width: `${storyResult.data.evaluation_score.Relevance * 100}%`,
              }"
            ></div>
          </div>
        </div>
        ...
        >
          ...
          <p class="text-2xl font-bold text-red-600">
            {{ storyResult.data.evaluation_score.Creativity.toFixed(3) }}
          </p>
          <div class="w-full bg-gray-200 rounded-full h-4">
            <div
              class="bg-red-600 h-4 rounded-full"
              :style="{
                width: `${storyResult.data.evaluation_score.Creativity * 100}%`,
              }"
            ></div>
          </div>
        </div>
...
const isScored = computed(() => {
  return (
    storyResult.value?.data.evaluation_score?.BLEU !== undefined &&
    storyResult.value?.data.evaluation_score?.Coherence !== undefined &&
    storyResult.value?.data.evaluation_score?.Relevance !== undefined &&
    storyResult.value?.data.evaluation_score?.Creativity !== undefined
  );
});
</script>
